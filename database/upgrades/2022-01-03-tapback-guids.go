...
const createMessageTable4 = `CREATE TABLE message (
...
const createTapbackTable4 = `CREATE TABLE tapback (
	chat_guid    TEXT,
	guid		 TEXT,
	message_guid TEXT,
	...
	mxid         TEXT NOT NULL UNIQUE,
	PRIMARY KEY (chat_guid, guid, message_guid, message_part, sender_guid),
...
func init() {
	upgrades[6] = upgrade{"Add a guid column to tapback table", func(tx *sql.Tx, c context) error {
		...
		_, err = tx.Exec(createMessageTable4)
		if err != nil {
			return fmt.Errorf("failed to create new message table: %w", err)
		}
		_, err = tx.Exec(createTapbackTable4)
		if err != nil {
		...
		_, err = tx.Exec("INSERT INTO tapback SELECT chat_guid, NULL, message_guid, message_part, sender_guid, type, mxid FROM old_tapback")
		if err != nil {
